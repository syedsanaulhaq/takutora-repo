        ...
        if($authorization_header != "" && strcasecmp($authorization_header, 'Basic' ) != 0 ){
			...
			if( isset($creds[0]) && isset($creds[1]) ) {
                ...
                if(\Drupal::config('rest_api_authentication.settings')->get('api_token') != $api_key ){
                    ...
                    $api_error = array('status' => 'error', 'code' => '401', "error" => "INVALID_API_KEY", 'error_description' => 'Sorry, you are using invalid API Key');
                }
                else if(empty($user)){
                    $config->set('miniorange_api_key_authentication_tried', "Failed")->save();
                    $api_error = array('status' => 'error', 'code' => '401', "error" => "USER_DOES_NOT_EXIST", 'error_description' => 'The user does not exists');
                }
                else{
                ...
                $config->set('miniorange_api_key_authentication_tried', "Failed")->save();
                $api_error = array('status' => 'error', 'code' => '401', "error" => "INVALID_AUTHORIZATION_HEADER", 'error_description' => 'The authorization header seems to be invalid');
            }
        }
        else{
            $config->set('miniorange_api_key_authentication_tried', "Failed")->save();
            $api_error = array('status' => 'error', 'code' => '401', "error" => "MISSING_AUTHORIZATION_HEADER", 'error_description' => 'The Authorization header is missing from the request.');
        }
        return $api_error;
    }
}
