...
class BusinessRulesUtil {
  ...
  public function getCurrentUri() {
   ...
   // Remove base path from the current request URI, but only at the front.
    $pattern = '/' . preg_quote(\Drupal::request()->getBasePath() . '/', '/') . '/';
  ...
  public function getPreviousUri() {
    try {
      ...
      +      // Remove base path from the referer URL, but only at the front.
      $pattern = '/' . preg_quote(\Drupal::request()->getBasePath() . '/', '/') . '/';
      $previousUrl = preg_replace($pattern,
        '/',
