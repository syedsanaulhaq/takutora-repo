...
function codesk_render_option_font(){
    $fonts = codesk_fonts();
    $output = '';
...
function codesk_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function codesk_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(codesk_fonts())) > 17){
...
function codesk_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode((string)$json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= codesk_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= codesk_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function codesk_options_patterns(){
    $output = '';
    $file_path = \Drupal::service('extension.list.theme')->getPath('codesk');
