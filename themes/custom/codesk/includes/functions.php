<?php
function codesk_render_block($key) {
...
function codesk_makeid($length = 5){
...
function codesk_base_url(){
  global $base_url;
  $theme_path = \Drupal::service('extension.list.theme')->getPath('codesk');
  return $base_url . '/' . $theme_path . '/';
}

global $gva_node_index;
function codesk_preprocess_node(&$variables) {
  global $gva_node_index;
  ...
  if ($variables['node']->getType() == 'article') {
      ...
      $variables['theme_uri'] = base_path() . \Drupal::service('extension.list.theme')->getPath('codesk');

  }
}

function codesk_preprocess_node__portfolio(&$variables){
  $node = $variables['node'];
...
function codesk_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function codesk_preprocess_breadcrumb(&$variables){
...
function codesk_print_icon($file, $alt){
